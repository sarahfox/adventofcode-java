...
public class UltraCityGrid {
    private int[][] cityGrid;
    private int maxX;
    private int maxY;
    // How many blocks can we go before we have to change direction?
    private static final int changeDirectionBlocks = 8;
    // What is the minimum number of blocks we have to go before changing direction?
    private static final int minimumChangeDirectionBlocks = 3;

    UltraCityGrid(List<String> lines) {
    ...
    public boolean isValidStep(Node currentNode, Direction heading) {
        ...
        // If the proposed heading is different than the current one, can we change direction?
        if(currentNode.direction != heading && currentNode.direction != Direction.START){
            return currentNode.blocks >= minimumChangeDirectionBlocks;
        }
