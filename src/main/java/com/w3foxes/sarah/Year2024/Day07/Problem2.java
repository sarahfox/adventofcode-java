...
public class Problem2 {
    ...
    public static boolean canBeSolvedRecursively(long answer, long intermediate, long[] remaining){
        ...
        long concat = Long.parseLong(String.valueOf(intermediate) + String.valueOf(remaining[0]));
        long[] lessRemaining = new long[remaining.length - 1];
        System.arraycopy(remaining,1,lessRemaining,0,remaining.length - 1);

        return canBeSolvedRecursively(answer, add, lessRemaining) 
            || canBeSolvedRecursively(answer, multiply, lessRemaining)
            || canBeSolvedRecursively(answer, concat, lessRemaining);
    }
}
